...
public enum MRTabBarItemPositioning: Int {
    case automatic
    case fill
    case centered
...
internal protocol MRTabBarDelegate: NSObjectProtocol {
    /// 当前item是否支持选中
    ///
    /// - Parameters:
    ///   - tabBar: tabBar
...
open class MRTabBar: UITabBar {
    internal weak var customDelegate: MRTabBarDelegate?
    ...
    public var itemCustomPositioning: MRTabBarItemPositioning? {
        didSet {
    ...
    internal var containers = [MRTabBarItemContainer]()
    ...
    open var moreContentView: MRTabBarItemContentView? = MRTabBarItemMoreContentView() {
        didSet {
            self.reload()
        }
    }
    ...
    open override func beginCustomizingItems(_ items: [UITabBarItem]) {
        MRTabBarController.printError("beginCustomizingItems(_:) is unsupported in ESTabBar.")
        super.beginCustomizingItems(items)
    }

    open override func endCustomizing(animated: Bool) -> Bool {
        MRTabBarController.printError("endCustomizing(_:) is unsupported in ESTabBar.")
...
internal extension MRTabBar /* Layout */ {
    func updateLayout() {
        guard let tabBarItems = items else {
            MRTabBarController.printError("empty items")
            return
        }
        ...
        let tabBarButtons = subviews.filter { subview -> Bool in
            ...
            subview1.frame.origin.x < subview2.frame.origin.x
        }
...
internal extension MRTabBar /* Actions */ {
    func isMoreItem(_ index: Int) -> Bool {
        return MRTabBarController.isShowingMore(tabBarController) && (index == (items?.count ?? 0) - 1)
    }
    ...
    func reload() {
        ...
        for (idx, item) in tabBarItems.enumerated() {
            let container = MRTabBarItemContainer(self, tag: 1000 + idx)
            addSubview(container)
            containers.append(container)

            if let item = item as? MRTabBarItem, let contentView = item.contentView {
    ...
    @objc func highlightAction(_ sender: AnyObject?) {
        guard let container = sender as? MRTabBarItemContainer else {
            return
        }
        ...
        if let item = item as? MRTabBarItem {
    ...
    @objc func select(itemAtIndex idx: Int, animated: Bool) {
        ...
        if currentIndex != newIndex {
            if currentIndex != -1, currentIndex < items?.count ?? 0 {
                if let currentItem = items?[currentIndex] as? MRTabBarItem {
    ...
    func updateAccessibilityLabels() {
        guard let tabBarItems = items, tabBarItems.count == containers.count else {
            return
        }
        ...
        for (idx, item) in tabBarItems.enumerated() {
            ...
            if let explicitLabel = item.accessibilityLabel {
                ...
                if isMoreItem(idx) {
                    accessibilityTitle = NSLocalizedString("More_TabBarItem", bundle: Bundle(for: MRTabBarController.self), comment: "")
                }
                ...
                let formatString = NSLocalizedString(item == selectedItem ? "TabBarItem_Selected_AccessibilityLabel" : "TabBarItem_AccessibilityLabel",
                                                     bundle: Bundle(for: MRTabBarController.self),
                                                     comment: "")
